...
#' @param timePeriod A string that gives the time period over which to average the data, e.g. c('1980-01', '2017-12')
...
plotNc <- function(long.name, nc.mod, mod.id, unit.conv.mod, variable.unit, timePeriod, outlier.factor = 1000, my.xlim = c(-180,
    ...
    raster::metadata(mod.mean) <- list(paste(variable.name, mod.id, "mod_mean", sep = "_"), paste("Mean", meta.data.mod, sep = "_"),
    ...
    # convert monthly to annual mean values
    indices <- format(as.Date(names(data), format = "X%Y.%m.%d"), format = "%Y")
    indices <- as.numeric(indices)
    indices <- indices - min(indices) + 1

    # annual mean values
    data <- raster::stackApply(data, indices, fun = mean)
    data.mean <- raster::mean(data, na.rm = TRUE)  # annual mean
    data.min <- min(data, na.rm = TRUE)  # annual min
    data.max <- max(data, na.rm = TRUE)  # annual max
    ...
    colnames(zonal.mean) <- c("lat", "min", "max", "mean")
    ...
    zone <- zonal.mean$lat
    zonal.mean.min <- zonal.mean$min
    zonal.mean.max <- zonal.mean$max
    ...
    # legend
    graphics::legend("topright", c("Annnual mean and total range"), col = c(my.col.mod), lty = 1, lwd = 2, bty = "n")
    # ticks
    ...
    # legend
    graphics::legend("topleft", "Monthly mean and total range", col = c(my.col.mod), lty = 1, lwd = 2, bty = "n")
    # ticks
    graphics::axis(4, labels = FALSE, tcl = 0.3)
