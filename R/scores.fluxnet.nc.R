...
scores.fluxnet.nc <- function(long.name, nc.mod, nc.ref, mod.id, ref.id, unit.conv.mod, unit.conv.ref, variable.unit, score.weights,
    ...
    # statistical analysis the time period of the reference data varies among sites some sites are located in the model ocean exclude
    # data in one data set that is not available in the other data set
    mask.mod <- (mod - mod + 1)
    mask.ref <- (ref - ref + 1)
    ...
    S_overall <- (w.bias * S_bias + w.rmse * S_rmse + w.phase * S_phase + w.iav * S_iav + w.dist * S_dist)/(w.bias + w.rmse + w.phase +
        w.iav + w.dist)
    ...
    # write data to file
    stat.metric <- data.frame(lon, lat, mod.mean, ref.mean, bias, bias.score, rmse, rmse.score, mod.max.month, ref.max.month, phase,
        phase.score, mod.iav, ref.iav, iav.score)
    colnames(stat.metric) <- c("lon", "lat", "mod.mean", "ref.mean", "bias", "bias.score", "rmse", "rmse.score", "mod.max.month",
